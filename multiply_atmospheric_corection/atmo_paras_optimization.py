...
class solving_atmo_paras(object):
    ...
    def _sort_emus_inputs(self, ):
        ...
        for i in [self.sza, self.vza, self.saa, self.vaa]:
            ...
            flat_angs_ele.append(
                [np.array(ang) for ang in np.ones(flat_boa.shape) * flat_i])  # make sure the type dose not change...
    ...
    def obs_cost(self, is_full=False):
        ...
        flat_mask, flat_boa, flat_toa, flat_boa_unc, flat_atmos, [sza, vza, saa, vaa,
                                                                  elevation] = self._sort_emus_inputs()
        ...
        H0, dH = self.AEE.emulator_reflectance_atmosphere(flat_boa, flat_atmos, sza, vza, saa, vaa, elevation,
        ...
        J = (
        0.5 * self.band_weights[..., None] * diff ** 2 / (self.band_weights[..., None].sum() * flat_boa_unc ** 2)).sum(
            axis=(0, 1))
        full_dJ = [
...
if __name__ == "__main__":
    ...
    print
    atmo.fmin_l_bfgs_cost([0.3, 3.2, 0.4], )
    print
    atmo.fmin_l_bfgs_cost([0.25, 3.2, 0.4], )
    atmo.optimization()
