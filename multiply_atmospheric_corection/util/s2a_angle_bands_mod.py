...
############################################################################
def s2a_angle(XML_File, subsamp=10):
    K0 = 0.9996
    ...
    try:
        ...
        def loop(tparms, AngleObs, gsd, subsamp, BandFoot, Orbit,  XML_File, sul_lat,sul_lon,slr_lat,slr_lon):
            ...
            zenith = np.zeros( (int(out_rows), int(out_cols)) )
            azimuth = np.zeros( (int(out_rows), int(out_cols)) )
            detcount = np.matrix( np.zeros( (int(out_rows), int(out_cols)) ) )
            ...
            for foot in BandFoot:
                    if foot['bandId'] == band:
                            ...
                            # sudipta changed above to support spatial subset
                            for row in range( int(ul_s_r), int(lr_s_r)):
                                    ...
                                    # sudipta changed above to support spatial subset
                                    for col in range( int(ul_s_c), int(lr_s_c)):
