...
class Kernels:
    ...
    def __integrateKernels(self, BSAangles=[]):
        ...
        for sza in self.BSAangles:
            # ross integral
            self.BSA_Ross[i], self.BSA_Ross_error[i] = scipy.integrate.dblquad(RossFunctionForIntegral, 0.0, 1.0, __gfun,
                                                                               __hfun, args=(sza, self))
            self.BSA_Li[i], self.BSA_Li_error[i] = scipy.integrate.dblquad(LiFunctionForIntegral, 0.0, 1.0, __gfun, __hfun,
                                                                           args=(sza, self))
            i = i + 1
    ...
    def printer(self, msg):
        ...
        if (self.FILE == -1):
            ...
            self.FILE.write(msg)  # some things required for the numerical integration
